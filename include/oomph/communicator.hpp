...
namespace oomph {
    ...
    public:
        ...
        template <typename T>
        message_buffer<T>
        make_device_buffer(T* device_ptr, std::size_t size, int id = hwmalloc::get_device_id())
        {
            return {make_buffer_core(device_ptr, size * sizeof(T), id), size};
        }

        template <typename T>
        message_buffer<T> make_device_buffer(
            T* ptr, T* device_ptr, std::size_t size, int id = hwmalloc::get_device_id())
        {
        ...
        template <typename T>
        send_multi_request
        send_multi(message_buffer<T> const& msg, std::vector<rank_type> const& neighs, tag_type tag)
        {
        ...
        template <typename T, typename CallBack>
        shared_recv_request
        shared_recv(message_buffer<T>&& msg, rank_type src, tag_type tag, CallBack&& callback)
        {
            OOMPH_CHECK_CALLBACK(CallBack)
            assert(msg);
            auto const s = msg.size();
        ...
        template <typename T, typename CallBack>
        send_request
        send(message_buffer<T> const& msg, rank_type dst, tag_type tag, CallBack&& callback)
        {
            ...
            std::vector<tag_type> tags, CallBack&& callback)
        {
            ...
            auto const s = msg.size();
            auto m_ptr = msg.m.m_heap_ptr.get();
            auto mrs = m_state->make_multi_request_state(
                std::move(neighs), std::move(tags), std::move(msg));
            auto const n = mrs->m_neighs.size();
            ...
            tag_type tag, CallBack&& callback)
        {
            ...
            {
                send(m_ptr, s * sizeof(T), dst, tag,
                    util::unique_function<void(rank_type, tag_type)>([mrs, callback](
                                                                         rank_type, tag_type t) {
                        if (--(mrs->m_counter) == 0ul)
                        {
    ...
    private:
        ...
        detail::message_buffer make_buffer_core(
        ...
        recv_request recv(detail::message_buffer::heap_ptr_impl* m_ptr, std::size_t size,
            rank_type src, tag_type tag, util::unique_function<void(rank_type, tag_type)>&& cb);

        shared_recv_request shared_recv(detail::message_buffer::heap_ptr_impl* m_ptr,
