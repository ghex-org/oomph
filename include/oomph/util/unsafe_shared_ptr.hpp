...
namespace oomph { namespace util {
    ...
    namespace detail {
        ...
        struct control_block_impl : public control_block<T>
        {
            ...
            using alloc_t =
                typename std::allocator_traits<Allocator>::template rebind_alloc<this_type>;
            ...
            template <typename D,
                std::enable_if_t<!std::is_base_of<enable_shared_from_this<D>, D>::value, bool> =
                    true>
    ...
    unsafe_shared_ptr<T> make_shared(Args&&... args)
    {
        return {std::allocator<char>{}, std::forward<Args>(args)...};
    }

    template <typename T, typename Alloc, typename... Args>
    unsafe_shared_ptr<T> allocate_shared(Alloc const& alloc, Args&&... args)
    {
