...
namespace oomph { namespace util {
    ...
    namespace detail {
        ...
        struct unique_function_impl : unique_function<R, Args...>
        {
            ...
            virtual R invoke(Args&&... args) final override
            {
        ...
        struct unique_function_impl<Func, void, Args...> : unique_function<void, Args...>
        {
            ...
            virtual void invoke(Args&&... args) final override
            {
    ...
    private:    // member types
        ...
        using buffer_t =
            std::aligned_storage_t<sbo_size, std::alignment_of<std::max_align_t>::value>;
    ...
    private:    // helper templates for type inspection
        ...
        template <typename F>
        using enable_if_large_function_t = std::enable_if_t<
            !is_unique_function_t<F>::value && (sizeof(std::decay_t<F>) > sbo_size), bool>;
        template <typename F>
        using enable_if_small_function_t = std::enable_if_t<
            !is_unique_function_t<F>::value && (sizeof(std::decay_t<F>) <= sbo_size), bool>;
    ...
    private:    // helper functions
        ...
        {
            return std::launder(
                reinterpret_cast<interface_t*>(&const_cast<buffer_t&>(std::get<2>(h))));
        }
