  ...
  public:
    ...
    virtual void OnTestCaseEnd(const TestCase& test_case) override
    {
        if (test_case_failures_)
        {
            printf_helper("[PASSED %3d; FAILED %3d] of %3d tests in %s\n\n",
                test_case_tests_ - test_case_failures_, test_case_failures_, test_case_tests_,
                test_case.name());
        }
        else
        {
            printf_helper("[ALL PASSED %3d; ] of %3d tests in %s\n\n", test_case_tests_,
    ...
    virtual void OnTestPartResult(const TestPartResult& test_part_result) override
    {
        ...
        printf_helper("  LOCAL_%s\n    %s\n    %s:%d\n%s\n    %s\n",
            test_part_result.failed() ? "FAIL" : "SUCCESS", banner, test_part_result.file_name(),
            test_part_result.line_number(), summary.c_str(), banner);

        // note that there was a failure in this test case
        if (test_part_result.failed()) { test_failures_++; }
    }
