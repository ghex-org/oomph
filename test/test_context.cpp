...
TEST_F(mpi_test_fixture, context_single)
{
    ...
    auto func = [&ctxt](int tid) {
        ...
        if (comm.rank() == 0)
        {
            ...
            for (unsigned int i = 0; i < size; ++i) msg[i] = i + payload_offset;
        }
        ...
        //
        //        if (comm_1.rank() == 0)
        //            while(counter_1 != comm_1.size()-1 or counter_2 != comm_2.size()-1) { comm_1.progress(); comm_2.progress(); }
        //
        //        if (comm_2.rank() != 0)
        //            fut_2.wait();
        //        if (comm_1.rank() != 0)
