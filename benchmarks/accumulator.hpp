...
namespace oomph {
    ...
    public:    // ctors
        ...
        accumulator(accumulator const&) noexcept = default;
        ...
        accumulator& operator=(accumulator const&) noexcept = default;
    ...
    public:    // add samples
        ...
        inline accumulator& operator()(value_type sample) noexcept
        {
            ...
            value_type const delta = sample - m_mean;
        ...
        /** @brief accumulate another accumulator 
              ...
              * @return reference to this object */
        inline accumulator& operator()(accumulator const& other) noexcept
        {
            ...
            auto const num_samples_new = m_num_samples + other.m_num_samples;
    ...
    public:
        ...
        operator<<(std::basic_ostream<CharT, Traits>& os, accumulator const& acc)
        {
    ...
    accumulator reduce(accumulator const& acc, MPI_Comm comm)
    {
        int rank, size;
        ...
        MPI_Gather(reinterpret_cast<void const*>(&acc), sizeof(accumulator), MPI_BYTE,
            reinterpret_cast<void*>(accs.data()), sizeof(accumulator), MPI_BYTE, 0, comm);
        accumulator acc_all;
        if (rank == 0)
        {
            for (auto const x : accs) { acc_all(x); }
        }
