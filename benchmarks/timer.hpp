...
namespace oomph {
    ...
    public:    // time functions
        ...
        {
            auto const t = std::chrono::duration_cast<std::chrono::microseconds>(
                clock_type::now() - m_time_point)
                               .count();
            this->operator()(t);
            return t;
        ...
        inline void vtoc(char const* header, long bytes) noexcept
        {
            double t = std::chrono::duration_cast<std::chrono::microseconds>(
                clock_type::now() - m_time_point)
                           .count();
    ...
    timer reduce(timer const& t, MPI_Comm comm)
    {
