...
cscs["daint"] = {
  ...
  "Run command": "srun --cpu-bind v,mask_cpu:0xffff --unbuffered --ntasks {total_ranks} --cpus-per-task {threads_per_rank}",
  "Batch preamble": """
#!/bin/bash -l
...
# this superceded by --cpu-bind v,mask_cpu:0xffff
# export GOMP_CPU_AFFINITY=0-{threadsm1}
...
#
def oomph_original(system, bin_dir, timeout, transport, progs, nodes, threads, msg, size, inflight, extra_flags="", env=""):
    total_ranks = 2
    whole_cmd = ''
...
def oomph_timed(system, bin_dir, timeout, transport, progs, nodes, threads, msg, size, inflight, extra_flags="", env=""):
    total_ranks = 2
    whole_cmd = ''
...
prog_arr  = ["bench_p2p_bi_cb_avail", "bench_p2p_bi_cb_wait",
             "bench_p2p_bi_ft_avail", "bench_p2p_bi_ft_wait"]
