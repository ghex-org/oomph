...
namespace oomph {
    ...
    communicator context::get_communicator()    //unsigned int tr)
    {
    ...
    rank_type context::rank() const noexcept { return m->rank(); }

    rank_type context::size() const noexcept { return m->size(); }

    rank_type context::local_rank() const noexcept { return m->topology().local_rank(); }

    rank_type context::local_size() const noexcept { return m->topology().local_size(); }

    char const* context::get_transport_option(std::string const& opt)
    {
        return m->get_transport_option(opt);
    }

    detail::message_buffer context::make_buffer_core(std::size_t size)
    {
        return m->get_heap().allocate(size, hwmalloc::numa().local_node());
    }

    detail::message_buffer context::make_buffer_core(void* ptr, std::size_t size)
    {
        return m->get_heap().register_user_allocation(ptr, size);
    }

#if OOMPH_ENABLE_DEVICE
    detail::message_buffer context::make_buffer_core(std::size_t size, int id)
    {
        return m->get_heap().allocate(size, hwmalloc::numa().local_node(), id);
    }

    detail::message_buffer context::make_buffer_core(
