...
class callback_queue2
{
  public: // member types
    ...
    struct element_type
    {
        ...
        element_type(cb_type&& cb, request_type&& req, handle_ptr&& h)
        : m_cb{std::move(cb)}
        , m_request{std::move(req)}
        , m_handle{std::move(h)}
        {
  ...
  public: // member functions
    void enqueue(request_type&& req, cb_type&& cb, handle_ptr&& h)
    {
        m_queue.push_back(element_type{std::move(cb), std::move(req), std::move(h)});
