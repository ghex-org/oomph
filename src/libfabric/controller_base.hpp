...
namespace NS_LIBFABRIC {
    ...
    struct stack_endpoint
    {
        ...
        ~stack_endpoint()
        {
            ...
            LF_DEB(NS_DEBUG::cnb_deb,
                trace(debug::str<>("Scalable Ep"), "used push", "ep", NS_DEBUG::ptr(get_ep()),
                    "tx cq", NS_DEBUG::ptr(get_tx_cq()), "rx cq", NS_DEBUG::ptr(get_rx_cq())));
    ...
    public:
        ...
        ~controller_base()
        {
            ...
            LF_DEB(NS_DEBUG::cnb_deb,
                debug(debug::str<>("counters"), "Received messages",
                    debug::dec<>(messages_handled_), "Total reads", debug::dec<>(rma_reads_),
                    "Total deletes", debug::dec<>(recv_deletes_), "deletes error",
            ...
            std::string const& provider, bool rootnode, int size, size_t threads, Args&&... args)
        {
            ...
            LF_DEB(NS_DEBUG::cnb_err,
                debug(debug::str<>("Rendezvous threshold"),
                    debug::hex<4>(msg_rendezvous_threshold_)));

            endpoint_type_ = static_cast<endpoint_type>(libfabric_endpoint_type());
            LF_DEB(
                NS_DEBUG::cnb_err, debug(debug::str<>("Endpoints"), libfabric_endpoint_string()));
    ...
    defined(HAVE_LIBFABRIC_EFA)
                ...
                for (unsigned int i = 0; i < threads_allocated; i++)
                {
                    ...
                    LF_DEB(NS_DEBUG::cnb_deb,
                        trace(debug::str<>("Scalable Ep"), "initial tx push", "ep",
                            NS_DEBUG::ptr(tx.get_ep()), "tx cq", NS_DEBUG::ptr(tx.get_tx_cq()),
                            "rx cq", NS_DEBUG::ptr(tx.get_rx_cq())));
        ...
        struct fid_ep* new_endpoint_active(struct fid_domain* domain, struct fi_info* info, bool tx)
        {
            ...
            fi_freeinfo(hints);
            LF_DEB(
                NS_DEBUG::cnb_deb, debug(debug::str<>("new_endpoint_active"), NS_DEBUG::ptr(ep)));
            return ep;
        }
        ...
        LF_DEB(NS_DEBUG::cnb_deb,
            ...
            LF_DEB(
                NS_DEBUG::cnb_deb, debug(debug::str<>("new_endpoint_scalable"), NS_DEBUG::ptr(ep)));
            fi_freeinfo(hints);
            return ep;
        }
                ...
                if (eps_->tl_tx_.get_ep() == nullptr)
                {
                    ...
                    LF_DEB(NS_DEBUG::cnb_deb,
                        trace(debug::str<>("Threadlocal Ep"), "create Tx", "ep",
                            NS_DEBUG::ptr(ep_tx), "tx cq", NS_DEBUG::ptr(tx_cq), "rx cq",
                            NS_DEBUG::ptr(nullptr)));
