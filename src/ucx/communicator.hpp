...
namespace oomph {
    ...
    public:
        ...
        void progress()
        {
            ...
            m_send_req_queue.consume_all([](detail::request_state* req) {
            ...
            if (m_thread_safe)
                m_recv_req_queue.consume_all([](detail::request_state* req) {
                    auto ptr = req->release_self_ref();
                    req->invoke_cb();
                });
            m_context->m_recv_req_queue.consume_all([](detail::shared_request_state* req) {
                auto ptr = req->release_self_ref();
                req->invoke_cb();
            });
        }
        ...
        {
            auto const& ep = m_send_worker->connect(dst);
            auto const stag =
            ...
            if (reinterpret_cast<std::uintptr_t>(ret) == UCS_OK)
            {
                ...
                else
                {
                    ...
                    auto s = m_req_state_factory.make(
                        m_context, this, scheduled, dst, tag, std::move(cb), ret, m_mutex);
                    s->create_self_ref();
            ...
            if (!UCS_PTR_IS_ERR(ret))
            {
                if (UCS_INPROGRESS != ucp_request_check_status(ret))
                {
                    ...
                    else
                    {
                        // allocate request_state
                        auto s = m_req_state_factory.make(
                            m_context, this, scheduled, src, tag, std::move(cb), ret, m_mutex);
                        s->create_self_ref();
                        // push callback to the queue
                        enqueue_recv(s.get());
                        return {std::move(s)};
            ...
            void* ucx_req, ucs_status_t status, ucp_tag_recv_info_t* /*info*/)
        {
            ...
            else if (status == UCS_ERR_CANCELED)
            {
                ...
                if (req_data.m_req)
                    req_data.m_req->m_comm->enqueue_cancel_recv(req_data.m_req);
                else
        ...
        bool cancel_recv(detail::request_state* s)
        {
            ...
            m_cancel_recv_req_queue.consume_all([this, s, &found](detail::request_state* r) {
                if (r == s)
                    found = true;
                else
