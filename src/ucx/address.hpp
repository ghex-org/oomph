...
namespace oomph {
    struct address_t
    {
        ...
        address_t(address_t const& other) = default;
        address_t& operator=(address_t const& other) = default;
        ...
        unsigned char const* data() const noexcept { return m_buffer.data(); }
        unsigned char* data() noexcept { return m_buffer.data(); }

        ucp_address_t const* get() const noexcept
        {
            return reinterpret_cast<ucp_address_t const*>(m_buffer.data());
        }
        ...
        operator<<(std::basic_ostream<CharT, Traits>& os, address_t const& addr)
        {
            os << "address{";
            os << std::hex;
