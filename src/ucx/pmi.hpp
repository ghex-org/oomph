...
namespace oomph {
    ...
    public:
        ...
        pmi(pmi const&) = delete;
        pmi(pmi&&) = default;
        pmi& operator=(pmi const&) = delete;
        ...
        ~pmi()
        {
            ...
            {
                OOMPH_WARN("Client ns %s rank %d:PMIx_Finalize failed: %d\n", myproc.nspace,
                    myproc.rank, rc);
            }
            else
            {
        ...
        void set(std::string const key, std::vector<unsigned char> const data)
        {
            int rc;
            pmix_value_t value;
        ...
        std::vector<unsigned char> get(uint32_t peer_rank, std::string const key)
        {
            int rc;
            pmix_proc_t proc;
            pmix_value_t* pvalue;
            ...
            if (PMIX_SUCCESS != (rc = PMIx_Get(&proc, key.c_str(), NULL, 0, &pvalue)))
            {
                std::string nspace(myproc.nspace, myproc.nspace + strlen(myproc.nspace));
                throw std::runtime_error("Client ns " + nspace + " rank " +
                    std::to_string(myproc.rank) + ": PMIx_Get " + key + ": " + std::to_string(rc) +
                    "\n");
            }
        ...
        void exchange()
        {
            ...
            if (PMIX_SUCCESS != (rc = PMIx_Fence(&allproc, 1, &info, 1)))
            {
                std::string nspace(myproc.nspace, myproc.nspace + strlen(myproc.nspace));
                throw std::runtime_error("Client ns " + nspace + " rank " +
                    std::to_string(myproc.rank) + ": PMIx_Fence failed: " + std::to_string(rc) +
                    "\n");
            }
