...
namespace oomph {
    struct worker_t
    {
        struct ucp_worker_handle
        {
            ...
            ucp_worker_handle(ucp_worker_handle const&) = delete;
            ucp_worker_handle& operator=(ucp_worker_handle const&) = delete;
            ...
            ucp_worker_h const& get() const noexcept { return m_worker; }
        };
        ...
        inline endpoint_t const& connect(rank_type rank)
        {
            auto it = m_endpoint_cache.find(rank);
            if (it != m_endpoint_cache.end()) return it->second;
            auto addr = m_db.find(rank);
            auto p = m_endpoint_cache.insert(
                std::make_pair(rank, endpoint_t{rank, m_worker.get(), addr}));
