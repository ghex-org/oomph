...
struct request_data
{
    ...
    static request_data* construct(void* ptr, Args&&... args)
    {
        // align pointer
        ...
        auto                            a_ptr = reinterpret_cast<request_data*>(
            ...
            mask);
        new (a_ptr) request_data{ptr, std::forward<Args>(args)...};
        return a_ptr;
    }
};
...
{
    std::memset(req, 0, request_data_size::value);
}
...
//struct request_cb
//{
//    using data_type = request_data_cb;
