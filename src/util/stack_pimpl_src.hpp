...
namespace oomph { namespace util {
    ...
    stack_pimpl<T, B, A> make_stack_pimpl(Args&&... args)
    {
    ...
    template <typename T, std::size_t B, std::size_t A>
    T* stack_pimpl<T, B, A>::operator->() noexcept
    {
        return m.get();
    }

    template <typename T, std::size_t B, std::size_t A>
    T const* stack_pimpl<T, B, A>::operator->() const noexcept
    {
        return m.get();
    }
    ...
    template <typename T, std::size_t B, std::size_t A>
    T* stack_pimpl<T, B, A>::get() noexcept
    {
        return m.get();
    }

    template <typename T, std::size_t B, std::size_t A>
    T const* stack_pimpl<T, B, A>::get() const noexcept
    {
        return m.get();
    }
