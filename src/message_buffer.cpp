...
namespace oomph { namespace detail {
    ...
    bool message_buffer::on_device() const noexcept { return m_heap_ptr->m.on_device(); }
    ...
    int message_buffer::device_id() const noexcept { return m_heap_ptr->m.device_id(); }

    void message_buffer::clone_to_device(std::size_t count)
    {
        hwmalloc::memcpy_to_device(m_heap_ptr->m.device_ptr(), m_ptr, count);
    }

    void message_buffer::clone_to_host(std::size_t count)
    {
