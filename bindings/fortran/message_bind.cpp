...
extern "C" void* oomph_message_new(std::size_t size, int allocator_type)
{
    ...
    default:
    {
        std::cerr << "BINDINGS: " << __FUNCTION__
                  << ": unsupported allocator type: " << allocator_type << "\n";
        std::terminate();
        break;
    }
    }
...
extern "C" void oomph_message_free(message_type** message_ref)
{
...
extern "C" void oomph_message_zero(message_type* message)
{
...
extern "C" unsigned char* oomph_message_data_wrapped(message_type* message, std::size_t* size)
{
    if (nullptr == message)
    {
        *size = 0;
